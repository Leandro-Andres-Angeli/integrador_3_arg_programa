...
VIEW `trailerflix`.`trailerflix_view` AS
    SELECT 
        ...
        (SELECT 
                GROUP_CONCAT(DISTINCT `trailerflix`.`generos`.`generos_nombre`
                        SEPARATOR ',')
            FROM
                ((`trailerflix`.`generos`
                JOIN `trailerflix`.`contenido_genero` ON ((`trailerflix`.`contenido_genero`.`genero_id` = `trailerflix`.`generos`.`generos_id`)))
                JOIN `trailerflix`.`contenido` ON ((`trailerflix`.`contenido_genero`.`contenido_id` = `c`.`contenido_id`)))) AS `genero`,
        `c`.`resumen` AS `resumen`,
        `c`.`temporadas` AS `temporadas`,
        `c`.`trailer` AS `trailer`,
        (SELECT 
                GROUP_CONCAT(DISTINCT `trailerflix`.`actores`.`actores_nombre`
                        SEPARATOR ',')
            FROM
                ((`trailerflix`.`actores`
                JOIN `trailerflix`.`contenido_actores` ON ((`trailerflix`.`contenido_actores`.`actores_id` = `trailerflix`.`actores`.`actores_id`)))
                JOIN `trailerflix`.`contenido` ON ((`trailerflix`.`contenido_actores`.`contenido_id` = `c`.`contenido_id`)))) AS `reparto`
    FROM
        (`trailerflix`.`contenido` `c`
        JOIN `trailerflix`.`categorias` `cat` ON ((`c`.`categoria_id` = `cat`.`categoria_id`)))    
